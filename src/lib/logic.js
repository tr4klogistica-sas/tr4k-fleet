...
export function calcPredictive(vehicle, maintenances = []) {
  ...
  const kmBase = vehicle?.km_compra || 0
  ...
  return getIntervals(vehicle).map(item => {
    ...
    let lastKm, nextKm
    if (last) {
      // Hay registro real — usarlo
      lastKm = last.km_al_momento
      nextKm = lastKm + item.h
    } else {
      // Sin registro: usar km_compra como base y avanzar al próximo ciclo futuro
      const cyclesPassed = Math.max(0, Math.floor((km - kmBase) / item.h))
      lastKm = kmBase + cyclesPassed * item.h
      nextKm = lastKm + item.h
    }

    const remaining = nextKm - km
    const pct       = Math.min(100, Math.max(0, Math.round((km - lastKm) / item.h * 100)))
