...
import { saveVehicle, uploadVehiclePhoto, addMaintenance, addKmLog, addCost, deleteMaintenance, deleteCost } from '../hooks/useSupabase.js'
import { calcPredictive, getIntervalsForVehicle, fmtKm, fmtCOP, fmtDate, daysUntil, docStatus } from '../lib/predictive.js'

const COST_TYPES = ['Salario conductor', 'Dotación', 'Combustible', 'Seguro', 'Parqueadero', 'Peajes', 'Lavado', 'Multa', 'Otro']
const EXTRA_TIPOS = ['Reparación eléctrica', 'Reparación mecánica', 'Revisión general', 'Otro']
...
export function HojaVida({ vehicle, maintenances, costs, onSaved }) {
  ...
  async function handleSave() {
    ...
    setSaving(true)
    const savedId = await saveVehicle(vehicle?.id || null, form)
    setSaving(false)
    setEditing(false)
    onSaved?.(savedId)
  }
            ...
            {maintenances.length === 0
              ...
              : <div className="timeline">{maintenances.map(m => (
                <div key={m.id} className="tl-row">
                  ...
                  <button className="btn-icon" title="Eliminar" style={{ fontSize: 11, opacity: .4, padding: '4px 7px' }}
                    onClick={() => { if (window.confirm('¿Eliminar este registro?')) deleteMaintenance(m.id) }}>✕</button>
                </div>
              ))}</div>
            }
          </div>
              ...
              : <table className="table">
                ...
                <tbody>{costs.map(c => (
                  <tr key={c.id}>
                    ...
                    <td><button className="btn-icon" title="Eliminar" style={{ fontSize: 11, opacity: .4, padding: '4px 7px' }}
                      onClick={() => { if (window.confirm('¿Eliminar este costo?')) deleteCost(c.id) }}>✕</button></td>
                  </tr>
                ))}</tbody>
              </table>
            }
...
export function Mantenimiento({ vehicles, allMaintenances, selectedId }) {
  ...
  const [form, setForm] = useState({ fecha: today, vehicle_id: selectedId || '' })
  const set = k => e => setForm(p => ({ ...p, [k]: e.target.value }))

  const selectedV = vehicles.find(v => v.id === form.vehicle_id)
  const brandIntervals = selectedV ? getIntervalsForVehicle(selectedV) : []
  const allTipos = [...brandIntervals.map(i => i.tipo), ...EXTRA_TIPOS]
  const selectedInterval = brandIntervals.find(i => i.tipo === form.tipo)
  ...
  async function handleSave() {
    ...
    setAdding(false)
    setForm({ fecha: today, vehicle_id: selectedId || '' })
  }
    ...
    <div className="fade-in">
      ...
      {adding && (
        <Modal title="Registrar mantenimiento" onClose={() => setAdding(false)} onSave={handleSave}>
          ...
          <div className="form-row">
            <Field label={`Tipo de mantenimiento${selectedV ? ` · ${selectedV.marca}` : ''}`}>
          ...
          {selectedInterval?.nota && (
            <div style={{ padding: '8px 12px', background: 'var(--surface-2)', borderRadius: 8, fontSize: 12, color: 'var(--text-2)', marginBottom: 12, borderLeft: '3px solid var(--gold)' }}>
              💡 <strong>{selectedV?.marca}:</strong> {selectedInterval.nota}
            </div>
          )}
          <div className="form-row">
...
// ══════════════════════════════════════════════════════════════════════════════
// IA PREDICTIVA — usa Claude API para analizar el historial y predecir desgaste
// ══════════════════════════════════════════════════════════════════════════════
export function IAPredict({ vehicles, allMaintenances, allKmLogs }) {
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState(null)
  const [selectedVId, setSelectedVId] = useState(vehicles[0]?.id || '')
  const [error, setError] = useState('')

  const vehicle = vehicles.find(v => v.id === selectedVId)
  const maintenances = allMaintenances[selectedVId] || []
  const kmLogs = allKmLogs[selectedVId] || []

  async function analyze() {
    if (!vehicle) return
    setLoading(true); setResult(null); setError('')

    // Construir contexto rico para el modelo
    const kmDiarios = kmLogs.slice(0, 30)
    const avgKmDia = kmDiarios.length > 0
      ? Math.round(kmDiarios.reduce((a, l) => a + (l.km_dia || 0), 0) / kmDiarios.length)
      : 0

    const historial = maintenances.slice(0, 15).map(m =>
      `- ${m.fecha}: ${m.tipo} a los ${m.km_al_momento?.toLocaleString('es-CO') || '?'} km. Taller: ${m.taller || 'N/A'}. Notas: ${m.notas || 'ninguna'}. Costo: $${m.costo?.toLocaleString('es-CO') || '0'} COP`
    ).join('\n')

    const intervals = getIntervalsForVehicle(vehicle)
    const intervalTable = intervals.map(i =>
      `  · ${i.tipo}: cada ${i.intervalo.toLocaleString('es-CO')} km${i.nota ? ` (${i.nota})` : ''}`
    ).join('\n')

    const prompt = `Eres un experto en mantenimiento de flotas de vehículos de carga en Colombia.
Analiza el siguiente vehículo y genera un análisis predictivo de desgaste detallado.

VEHÍCULO:
- Placa: ${vehicle.placa}
- Marca/Modelo: ${vehicle.marca} ${vehicle.modelo} (${vehicle.ano})
- Km actuales: ${(vehicle.km_actual || 0).toLocaleString('es-CO')} km
- Km promedio diario: ${avgKmDia} km/día
- Conductor: ${vehicle.conductor || 'No registrado'}

PLAN DE MANTENIMIENTO OFICIAL PARA ESTE VEHÍCULO (${vehicle.marca}):
${intervalTable}

HISTORIAL DE MANTENIMIENTOS REGISTRADOS (${maintenances.length} registros):
${historial || 'Sin historial registrado aún. Usar el plan oficial como referencia base.'}

Con base en el plan oficial del fabricante, el historial real y el uso diario, genera:

1. **DIAGNÓSTICO GENERAL** — Estado actual del vehículo en 2-3 frases concretas.

2. **PREDICCIÓN DE DESGASTE** — Para cada componente crítico según el plan oficial:
   Indica estado estimado, km restantes antes de intervención, nivel de riesgo: BAJO / MEDIO / ALTO / CRÍTICO

3. **PRÓXIMAS 3 INTERVENCIONES PRIORITARIAS** — Con fecha estimada basada en ${avgKmDia} km/día promedio.

4. **ALERTAS ESPECÍFICAS** — Basadas en el historial: patrones detectados, componentes que se han retrasado, puntos de atención especiales para el ${vehicle.marca} ${vehicle.modelo}.

5. **COSTO ESTIMADO** — Mantenimiento proyectado para los próximos 3 meses en COP colombianos.

Sé específico. Usa los datos reales. Responde en español.`
  ...
  // Parsea el resultado en secciones para renderizado visual
  function renderResult(text) {
    ...
    return sections.map((section, i) => {
      ...
      const isRisk = title.includes('PREDICCIÓN') || title.includes('DESGASTE')
      return (
        <div key={i} className="card mb-4">
          <div className="card-header">
            <div className="card-title-lg" style={{ color: 'var(--text-1)' }}>
              {title.replace(/\*\*/g, '')}
            </div>
          </div>
          <div style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--text-2)', whiteSpace: 'pre-wrap' }}>
            {content.split('\n').map((line, j) => {
              // Resalta líneas con nivel de riesgo
              const isCritical = line.includes('CRÍTICO')
              const isHigh = line.includes('ALTO') && !isCritical
              const isMed = line.includes('MEDIO')
              const isLow = line.includes('BAJO')
              const color = isCritical ? 'var(--red)' : isHigh ? 'var(--amber)' : isMed ? '#854F0B' : isLow ? 'var(--green)' : 'inherit'
              const bold = isCritical || isHigh
              return (
                <div key={j} style={{ color, fontWeight: bold ? 500 : 400, marginBottom: line.trim() ? 2 : 6 }}>
                  {line.replace(/\*\*/g, '')}
                </div>
              )
            })}
  ...
  return (
    <div className="fade-in">
      <div className="topbar">
        <div>
          <div className="page-title">Diagnóstico IA · Desgaste Predictivo</div>
          <div className="page-sub">Análisis inteligente basado en historial real del vehículo</div>
        </div>
      </div>

      <div className="content">
        {/* Selector de vehículo + botón */}
        <div className="card mb-4">
          <div className="card-title" style={{ marginBottom: 14 }}>Selecciona el vehículo a analizar</div>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
            <div style={{ flex: 1, minWidth: 200 }}>
              <div className="field">
                <label>Vehículo</label>
                <select value={selectedVId} onChange={e => { setSelectedVId(e.target.value); setResult(null) }}>
                  {vehicles.map(v => <option key={v.id} value={v.id}>{v.placa} · {v.marca} {v.modelo} · {(v.km_actual || 0).toLocaleString('es-CO')} km</option>)}
                </select>
              </div>
            </div>
            {vehicle && (
              <div style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: 13, color: 'var(--text-3)', flexWrap: 'wrap' }}>
                <span>📊 {(allKmLogs[selectedVId] || []).slice(0,30).length} días de km registrados</span>
                <span>🔧 {maintenances.length} mantenimientos en historial</span>
              </div>
            )}
            <button
            ...
            >
              {loading ? '⏳ Analizando...' : '🤖 Generar diagnóstico IA'}
            </button>
          </div>

          {maintenances.length === 0 && (
            <div style={{ marginTop: 14, padding: '12px 14px', background: 'var(--amber-bg)', borderRadius: 8, fontSize: 13, color: 'var(--amber)' }}>
              ⚠ Este vehículo no tiene mantenimientos registrados. El análisis será más preciso cuantos más registros tengas. Puedes generarlo igual como referencia general.
            </div>
          )}
        </div>
        ...
        {loading && (
          <div className="card" style={{ textAlign: 'center', padding: 48 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 500, marginBottom: 8 }}>Analizando historial del vehículo...</div>
            <div style={{ fontSize: 13, color: 'var(--text-3)' }}>La IA está procesando {maintenances.length} mantenimientos y el patrón de uso diario</div>
            <div style={{ marginTop: 20 }}>
              <div style={{ width: 200, margin: '0 auto', height: 4, background: 'var(--surface-2)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{ height: '100%', background: 'var(--gold)', borderRadius: 2, animation: 'loading 1.5s ease-in-out infinite' }} />
              </div>
            </div>
        ...
        {error && (
          <div className="alert alert-danger">
            <div className="alert-icon">⚠</div>
            <div className="alert-body">
              <div className="alert-title">Error al generar el diagnóstico</div>
        ...
        {result && !loading && (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, padding: '12px 16px', background: 'var(--gold-dim)', borderRadius: 10, border: '1px solid rgba(184,134,11,0.2)' }}>
              <span style={{ fontSize: 20 }}>🤖</span>
              <div>
                <div style={{ fontSize: 13, fontWeight: 500 }}>Diagnóstico generado por IA · {vehicle?.placa}</div>
                <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Basado en {maintenances.length} registros históricos · {new Date().toLocaleDateString('es-CO')}</div>
              </div>
              <button className="btn btn-ghost btn-sm" style={{ marginLeft: 'auto' }} onClick={() => setResult(null)}>Cerrar</button>
            </div>
            {renderResult(result)}
          </>
        )}
        ...
        {!result && !loading && !error && (
          <div className="card" style={{ textAlign: 'center', padding: 60 }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🧠</div>
            <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 8 }}>Diagnóstico predictivo con IA</div>
            <div style={{ fontSize: 13, color: 'var(--text-3)', maxWidth: 400, margin: '0 auto', lineHeight: 1.7 }}>
              La IA analiza el historial de mantenimientos, el patrón de km diarios y las características del vehículo para predecir el desgaste de cada componente y las próximas intervenciones necesarias.
            </div>
            <div style={{ marginTop: 24, display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
              {['Desgaste por componente', 'Fechas estimadas', 'Costos proyectados', 'Riesgos detectados'].map(t => (
                <div key={t} style={{ padding: '6px 12px', background: 'var(--surface-2)', borderRadius: 20, fontSize: 12, color: 'var(--text-2)' }}>✓ {t}</div>
              ))}
            </div>
          </div>
        )}
