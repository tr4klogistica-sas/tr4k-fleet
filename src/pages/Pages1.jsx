...
export function Vehiculos({ vehicle, maintenances, costs, onSaved }) {
  ...
  async function handleSave() {
    ...
    setSaving(false); setEditing(false); onSaved?.(id)
  }
  ...
  if (!vehicle?.id && !editing) return (
    <div className="content">
      <Empty icon="🚛" title="Selecciona un vehículo" sub="O crea uno nuevo"
        action={<button className="btn btn-p" onClick={() => setEditing(true)}>+ Nuevo vehículo</button>} />
    </div>
  )
    ...
    <div className="fade-in">
      <div className="topbar">
        ...
        <div className="top-actions">
          <button className="btn btn-g" onClick={() => { setForm(vehicle || empty); setEditing(true) }}>✏ Editar</button>
